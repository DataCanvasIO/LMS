...
def main(args):
    ...
    if args.test_before_train:
        logger.log("===================================================================")
        logger.log("==================Generation Results Before Pruning================")
        logger.log("===================================================================")
        model.eval()
        with torch.no_grad():
    ...
    if args.block_wise:
        ...
        kwargs = {
            ...
            "ch_sparsity_dict": {
                model.transformer.h[i].self_attention.query_key_value: args.pruning_ratio / 3 for i in
            ...
            "root_instances": [model.transformer.h[i].mlp.dense_h_to_4h for i in
                               range(args.block_mlp_layer_start, args.block_mlp_layer_end)] +
                              [model.transformer.h[i].self_attention.query_key_value for i in
                               range(args.block_attention_layer_start, args.block_attention_layer_end)],
    ...
    if args.save_model:
        ...
        from . import util
        util.save_json(logger.best_checkpoint_path, "model", "tokenizer")
    ...
    if args.test_after_train:
        logger.log("===================================================================")
        logger.log("==================Generation Results After Pruning=================")
        logger.log("===================================================================")
        model.eval()
        with torch.no_grad():
            for prompt in prompts:
